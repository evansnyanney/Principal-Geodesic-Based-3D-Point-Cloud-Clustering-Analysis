...
def log_map(points, base_point):
    ...
    tangent_vectors = (points - np.outer(dot_product, base_point)) / sin_angles[
        :, np.newaxis
    ]
...
def compute_pga(cluster_points, mean, method="minmax", n_components=3):
    ...
    return {"principal_directions": principal_directions, "eigenvalues": eigenvalues}


def compute_frechet_mean_per_cluster(
    all_scans, scans_cluster_labels, n_clusters, method="minmax"
):
