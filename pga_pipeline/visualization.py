...
def plot_variance_distributions_single_figure(
    cluster_variance_stats,
    n_clusters,
    cluster_colors,
    output_folder="outputs/variance_distributions",
):
    ...
    for cluster, stats in cluster_variance_stats.items():
        df = pd.DataFrame(
            stats["all_variances"], columns=["Variance_X", "Variance_Y", "Variance_Z"]
        )
        df["Cluster"] = cluster
        data.append(df)
    ...
    titles = [
        "Variance in X Across Clusters",
        "Variance in Y Across Clusters",
        "Variance in Z Across Clusters",
    ]
...
def plot_clusters_visualization(
    points,
    cluster_labels,
    scan_idx,
    cluster_colors,
    output_folder="outputs/cluster_visualizations",
):
    os.makedirs(output_folder, exist_ok=True)
    ...
    if -1 in unique_clusters:
        ...
        adjusted_norm = BoundaryNorm(
            np.arange(-0.5, n_clusters + 1.5, 1), ncolors=n_clusters + 1
        )
    else:
        adjusted_cmap = cmap
        adjusted_norm = norm
    ...
    ax1.set_title(
        f"Scan {scan_idx}: Cluster Visualization X vs Y", fontsize=32, fontweight="bold"
    )
    ...
    ax2.set_title(
        f"Scan {scan_idx}: Cluster Visualization Y vs Z", fontsize=32, fontweight="bold"
    )
    ...
    ax3.set_title(
        f"Scan {scan_idx}: Cluster Visualization X vs Z", fontsize=32, fontweight="bold"
    )
    ...
    ax4.set_title(
        f"Scan {scan_idx}: 3D Cluster Visualization", fontsize=32, fontweight="bold"
    )
    ...
    plt.savefig(
        os.path.join(output_folder, f"Cluster_Visualization_Scan_{scan_idx}.png"),
        dpi=300,
    )
    plt.close()
...
def plot_average_variance(
    points,
    cluster_labels,
    average_variance_stats,
    method="frechet_mean",
    n_clusters=7,
    cluster_colors=None,
    output_folder="outputs/average_variance_plots",
):
    os.makedirs(output_folder, exist_ok=True)
    ...
    ax1.set_title(
        f"{method} Average Variance Visualization X vs Y",
        fontsize=32,
        fontweight="bold",
    )
    ax1.set_xlabel("X Coordinate", fontsize=28, fontweight="bold")
    ...
    ax2.set_title(
        f"{method} Average Variance Visualization Y vs Z",
        fontsize=32,
        fontweight="bold",
    )
    ax2.set_xlabel("Y Coordinate", fontsize=28, fontweight="bold")
    ...
    ax3.set_title(
        f"{method} Average Variance Visualization X vs Z",
        fontsize=32,
        fontweight="bold",
    )
    ax3.set_xlabel("X Coordinate", fontsize=28, fontweight="bold")
    ...
    ax4.set_title(
        f"{method} Average Variance Visualization (3D)", fontsize=32, fontweight="bold"
    )
    ax4.set_xlabel("X Coordinate", fontsize=28, fontweight="bold", labelpad=20)
    ax4.set_ylabel("Y Coordinate", fontsize=28, fontweight="bold", labelpad=20)
    ax4.set_zlabel("Z Coordinate", fontsize=28, fontweight="bold", labelpad=40)
    ...
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(
        os.path.join(output_folder, f"{method}_Average_Variance_Visualization.png"),
        dpi=300,
    )
    plt.close()
    ...
    for cluster, stats in cluster_pga_stats.items():
        ...
        for i, direction in enumerate(principal_directions):
            ax.quiver(
                ...
                direction[1],
                direction[2],
                length=eigenvalues[i] * 2,
                color="r",
                arrow_length_ratio=0.1,
                linewidth=2,
            )
        ax.set_title(
            f"Cluster {cluster}: Principal Geodesic Directions",
            fontsize=32,
            fontweight="bold",
        )
...
def plot_means(
    ...
    n_clusters=7,
    cluster_colors=None,
    output_folder="outputs/means_visualizations",
):
    os.makedirs(output_folder, exist_ok=True)
    palette = cluster_colors[:n_clusters]
    cmap = ListedColormap(palette)
    ...
    cbar4 = fig.colorbar(
        scatter4, ax=ax4, shrink=0.6, aspect=20, pad=0.15, ticks=range(n_clusters)
    )
    cbar4.set_label("Cluster ID", fontsize=28, fontweight="bold")
    cbar4.ax.tick_params(labelsize=28)
    ...
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(
        os.path.join(output_folder, f"{method}_Means_Visualization.png"), dpi=300
    )
    plt.close()
