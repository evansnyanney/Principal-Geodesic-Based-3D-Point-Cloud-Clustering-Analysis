...
from pga_pipeline.geometry import (
    normalize_points,
    compute_frechet_mean,
    compute_euclidean_mean,
    compute_pga,
    compute_frechet_mean_per_cluster,
)
from pga_pipeline.clustering import (
    cluster_point_cloud_spectral,
    assess_clustering_consistency,
)
from pga_pipeline.stats import (
    associate_variance_with_clusters,
...
logging.basicConfig(
    filename="outputs/local_pga_validation.log",
    level=logging.INFO,
    ...
    try:
        ...
        for idx, scan in enumerate(all_scans, start=1):
            if scan.shape[0] != num_landmarks:
                logging.error(
                    f"Scan {idx} has {scan.shape[0]} landmarks, expected {num_landmarks}."
                )
                sys.exit(1)
        ...
        variance_data = load_pointwise_variance(
            variance_file_path,
            variance_columns=[
        ...
        for scan_idx, scan in enumerate(tqdm(all_scans, desc="Processing scans")):
            ...
            cluster_labels = cluster_point_cloud_spectral(
                normalized_scan, n_clusters=k_clusters, n_neighbors=n_neighbors
            )
            scans_cluster_labels.append(cluster_labels)

            print("Associating variance with clusters...")
            cluster_variance_stats = associate_variance_with_clusters(
                cluster_labels, variance_data
            )
            ...
            for cluster_id, stats in cluster_variance_stats.items():
                ...
                logging.info(
                    f"Mean Variance X: {stats['mean_variance'][0]:.6f}, Y: {stats['mean_variance'][1]:.6f}, Z: {stats['mean_variance'][2]:.6f}"
                )
                logging.info(
                    f"Std Dev Variance X: {stats['std_variance'][0]:.6f}, Y: {stats['std_variance'][1]:.6f}, Z: {stats['std_variance'][2]:.6f}"
                )
            ...
            plot_clusters_visualization(
                ...
                scan_idx + 1,
                CLUSTER_COLORS,
                output_folder="results/cluster_visualizations",
            )
            ...
            print(
                f"Finished processing Scan {scan_idx + 1} in {elapsed_time:.2f} seconds."
            )
        ...
        means = compute_frechet_mean_per_cluster(
            all_scans, scans_cluster_labels, n_clusters, method=NORMALIZATION_METHOD
        )
        ...
        combined_variance_df.to_csv(
            "results/combined_within_cluster_variances.csv", index=False
        )
        ...
        plot_variance_distributions_single_figure(
            ...
            n_clusters,
            CLUSTER_COLORS,
            output_folder="results/variance_distributions",
        )
        ...
        plot_means(
            ...
            output_folder="results/means_visualizations",
        )
