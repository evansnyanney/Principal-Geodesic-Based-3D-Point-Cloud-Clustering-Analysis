...
# Configure logging
logging.basicConfig(filename='clustering_variance_analysis.log',
                    level=logging.INFO,
...
def main():
    ...
    scan_files = sorted([f for f in os.listdir(scans_folder_path) if f.endswith('.csv')])
    if not scan_files:
    ...
    k_clusters = 5
    n_neighbors = 10
    cluster_labels = cluster_point_cloud_spectral(points, n_clusters=k_clusters, n_neighbors=n_neighbors)
    ...
    variance_components = ['Pointwise Variance X', 'Pointwise Variance Y', 'Pointwise Variance Z']
    ...
    for variance_component in variance_components:
        ...
        for cluster_id in sorted(variance_df['Cluster'].unique()):
            ...
            # Plot histogram
            sns.histplot(cluster_data[variance_component], bins=30, kde=False,
                         color=cluster_colors[cluster_id], label=f'Cluster {cluster_id}', alpha=0.5)
